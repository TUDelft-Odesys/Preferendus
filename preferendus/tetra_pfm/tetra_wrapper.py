"""
Code to send to and receive data from teh Tetra API endpoint. Code is an python
adaptation of the Matlab code written by Dmitry Zhilyaev, TU Delft
(D.Zhilyaev@tudelft.nl).

Credentials are needed for the API. The credentials are not free to share.

Copyright (c) 2022. Harold Van Heukelum & Dmitry Zhilyaev
"""
import pathlib
...
class TetraSolver:
    """
    Class to interact with the web version of Tetra. Builds the XML used as input for
    Tetra and returns the output of Tetra as an array with floats.
    """
    ...
    def __init__(self):
        ...
        try:
            ...
            print(
                "Username and password are not found (missing .env file). Please fill "
                "them in now:"
            )
    ...
    def _indent(self, elem, level=0):
        """
        Credits to Erick M. Sprengel for this method's code. Source:
        https://bit.ly/3IpXZHP
    ...
    def _make_xml(self, w, p):
        """
        Method to build the XML file that is sent to the Tetra server. The basis of the
        document is the template document.
    ...
    def request(self, w, p):
        """
        Method that handles the communication with the Tetra server. Gathers xml
        ElementTree from method make_xml and returns an array with the values that are
        returned from Tetra.
        ...
        xml_tree = Et.tostring(
            self._make_xml(w=w, p=p).getroot(), encoding="utf8", method="xml"
        )
        ...
        Et.ElementTree(tree).write(
            f"{HERE}/tetra_out.xml", encoding="utf-8", xml_declaration=True
        )
    ...
    def assertion_tests(w, p):
        ...
        assert (
            round(sum(w), 4) == 1
        ), f"Sum of weights should be equal to 1. They are now sum({w})={sum(w)}."
        return
