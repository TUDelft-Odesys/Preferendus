"""
Python code for the rail level-crossing service life design problem.
...
from scipy.interpolate import pchip_interpolate, RectBivariateSpline
...
data_acc = np.loadtxt(
    "data/Data_acceleration_2d.csv", delimiter=";", encoding="utf-8-sig"
)
...
def objective_maintenance_costs(
    force: np.ndarray[float], acc: np.ndarray[float]
) -> np.ndarray[float]:
...
def single_objective_p1(variables: np.ndarray) -> np.ndarray[float]:
    ...
    for ix in range(len(var1)):
        force.append(
            force_inter(var1[ix], var2[ix])[0][0]
        )  # get force on rail for every member of the population
        acc.append(
            acc_inter(var1[ix], var2[ix])[0][0]
        )  # get acceleration of rail for every member of the population

    return objective_maintenance_costs(np.array(force), np.array(acc))


def single_objective_p2(variables: np.ndarray) -> np.ndarray[float]:
...
def single_objective_p3(variables: np.ndarray) -> np.ndarray[float]:
...
def objective(variables: np.ndarray) -> tuple[list, list]:
    ...
    return [W_1, W_2, W_3], [p_1, p_2, p_3]


def print_results(x):
    """Function that prints the results of the optimizations"""
    print(
        f"Optimal result for x1 = {round(x[0], 2)}m and x2 = {round(x[1], 2)} sleepers"
    )


def make_figures(
    variables: np.ndarray,
    objective_results: list,
    ...
    for i in range(len(variables)):
        ax.scatter(variables[i, 0], variables[i, 1], label=method[i], marker=markers[i])
    ...
    for i in range(len(objective_results[2])):
        ax3.scatter(
            objective_results[2][i] * 1e-3, preference_results[2][i], marker=markers[i]
        )
    ax3.set_ylim((0, 100))
...
if __name__ == "__main__":
    ...
    # maintenance costs
    ga = Preferendus(
        objective=single_objective_p1, constraints=(), bounds=bounds, options=options
    )
    ...
    # riding comfort
    ga = Preferendus(
        objective=single_objective_p2, constraints=(), bounds=bounds, options=options
    )
    ...
    # investment costs
    ga = Preferendus(
        objective=single_objective_p3, constraints=(), bounds=bounds, options=options
    )
    ...
    for i in range(len(variable)):
        f_res.append(
            force_inter(variable[:, 0][i], variable[:, 1][i])[0][0]
        )  # get force on rail
        a_res.append(
            acc_inter(variable[:, 0][i], variable[:, 1][i])[0][0]
        )  # get acceleration of rail
    ...
    d = {
        "Method": methods,
        "Maintenance costs": np.round(c1_res, 2),
        "Travel comfort": np.round(c2_res, 2),
        "Investment costs": np.round(c3_res, 2),
    }
    print()
    print(pd.DataFrame(data=d).to_string())
    print()
    ...
    d = {
        "Method": methods,
        "Maintenance costs": np.round(p1_res),
        "Travel comfort": np.round(p2_res),
        "Investment costs": np.round(p3_res),
    }
    print()
    print(pd.DataFrame(data=d).to_string())
    print()

    make_figures(variable, [c1_res, c2_res, c3_res], [p1_res, p2_res, p3_res], methods)
